...
- **Root-level data pull scripts** (occasionally used for manual data updates):
  ...
  Email notification subsystem:
    - `scripts/generate_driver_constructor_map.py` — creates a canonical `data_files/driver_to_constructor.csv` mapping used to fill missing constructor names in headless predictions.
    - `scripts/headless_predict_and_write.py` — lightweight headless prediction generator used as a fallback when per-GP predictions are missing. Writes `data_files/predictions_headless_next-race_<year>.csv`.
    - `scripts/export_email_context.py` — exporter that builds an inline HTML snippet and a TSV attachment for email. Auto-selects a predictions file when `--input` is not provided (calendar-based next-race heuristics, then newest predictions). Produces `data_files/email_context_<gp>_<year>.html` and `data_files/email_context_<gp>_<year>.tsv`. Detects and includes MAE when present.
    - `scripts/send_rich_email_now.py` — SMTP sender wrapper that embeds the HTML snippet and attaches the TSV. Important behaviour: it calls the exporter unconditionally before each send so the inline snippet and attachment are regenerated for every send. Uses `.env` for SMTP/credential configuration.
    - `notifications/templates/race_results.html.j2` — Jinja2 template used by the sender; updated to avoid duplicated Race/Date headers when the exporter provides an inline snippet.
    - `data_files/driver_to_constructor.csv` — canonical mapping generated by `generate_driver_constructor_map.py`.
    - `data_files/email_context_*.tsv` / `.html` — exporter outputs for embedding/attachment.
    - `data_files/predictions_headless_*.csv` — headless fallback prediction artifacts.

  Notes on behaviour and best-practices for email sends:
