...
def _read_json_safe(filepath: str) -> pd.DataFrame:
    """Read a JSON file into a DataFrame, using stdlib json for single-line
    files to avoid ujson MemoryError on large payloads."""
    with open(filepath, 'r', encoding='utf-8') as f:
        first_char = f.read(1)
        if first_char != '[':
            f.seek(0)
            return pd.read_json(f)
        # Check if it's a single-line array (the problem case)
...
drivers = _read_json_safe(path.join(DATA_DIR, 'f1db-drivers.json')) 

# Exclude 'jos-verstappen' as he is not Max Verstappen
drivers = drivers[drivers['id'] != 'jos-verstappen']

race_results = _read_json_safe(path.join(DATA_DIR, 'f1db-races-race-results.json')) 
races = _read_json_safe(path.join(DATA_DIR, 'f1db-races.json')) 
constructors = _read_json_safe(path.join(DATA_DIR, 'f1db-constructors.json')) 
qualifying_json = _read_json_safe(path.join(DATA_DIR, 'f1db-races-qualifying-results.json'))
# Note: all_qualifying_races.csv is not used (empty/broken) - we use f1db JSON directly
grandPrix = _read_json_safe(path.join(DATA_DIR, 'f1db-grands-prix.json')) 
fp1 = _read_json_safe(path.join(DATA_DIR, 'f1db-races-free-practice-1-results.json')) 
fp2 = _read_json_safe(path.join(DATA_DIR, 'f1db-races-free-practice-2-results.json')) 
fp3 = _read_json_safe(path.join(DATA_DIR, 'f1db-races-free-practice-3-results.json')) 
# fp4 = _read_json_safe(path.join(DATA_DIR, 'f1db-races-free-practice-4-results.json')) 
current_practices = pd.read_csv(get_preferred_file('all_practice_laps.csv'), sep='\t') 
# Prefer a cleaned/filled practice-best file if present (generated by helper scripts)
preferred_practice_best_candidates = [
    path.join(DATA_DIR, 'practice_best_by_session.imputed.round_filled.csv'),
    path.join(DATA_DIR, 'practice_best_by_session.imputed.csv'),
    path.join(DATA_DIR, 'practice_best_by_session.csv'),
]
practice_best_path = None
for cand in preferred_practice_best_candidates:
    if path.exists(cand):
        practice_best_path = cand
        break
if practice_best_path is None:
    # fall back to the general preferred file selection for legacy names
    practice_best_path = get_preferred_file('practice_best_fp1_fp2.csv')
practice_best = pd.read_csv(practice_best_path, sep='\t')

# Ensure `round` is populated: prefer existing values, then use race-level metadata from `races`.
if 'round' not in practice_best.columns:
    practice_best['round'] = np.nan
if practice_best['round'].isna().any():
    # Build a mapping from race id -> round from the loaded `races` JSON (column 'id')
    if 'id' in races.columns and 'round' in races.columns:
        try:
            race_round_map = races.set_index('id')['round'].to_dict()
            # Some raceId values may be floats/strings; coerce to match types where possible
            practice_best['round'] = practice_best.apply(
                lambda r: r['round'] if not pd.isna(r['round']) else race_round_map.get(r['raceId'], np.nan),
                axis=1,
            )
        except Exception:
            # Best-effort: leave as-is if mapping fails
            pass
pitstops = _read_json_safe(path.join(DATA_DIR, 'f1db-races-pit-stops.json'))
...
# Suppress noisy debug output for Andrea/Kimi Antonelli rows. To enable printing set env var
# `DEBUG_PRINT_ANTONELLI=1` before running.
# Removed noisy Antonelli debug print (was used during data-repair).
...
races = _read_json_safe(path.join(DATA_DIR, 'f1db-races.json')) 
circuits = _read_json_safe(path.join(DATA_DIR, 'f1db-circuits.json')) 
